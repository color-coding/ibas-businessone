...
LC_ALL=C sed 's/\r//g' "${WORK_FOLDER}/compile_order.txt" | while IFS= read -r line; do
  if [ -e ${WORK_FOLDER}/${line}/pom.xml ]; then
    ...
    if [ ${isService} != 0 ]; then
      # 网站，编译war包
      echo --开始编译[${line}]
      mvn -q clean package ${MAVEN_PACKAGE_ARGUMENTS} -Dmaven.test.skip=true -f ${WORK_FOLDER}/${line}/pom.xml
      ...
      # 非网站，编译jar包并安装到本地
      echo --开始编译[${line}]+安装
      mvn -q clean package install ${MAVEN_PACKAGE_ARGUMENTS} -Dmaven.test.skip=true -f ${WORK_FOLDER}/${line}/pom.xml
...
chmod +x ${WORK_FOLDER}/release/btulz.shell.sh
echo --压缩编译文件为tar包
if [ -e ${WORK_FOLDER}/release ]; then
